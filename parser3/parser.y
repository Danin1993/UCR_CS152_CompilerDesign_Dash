...
%type <codenode> paramerter_dec return_stm
...
functions  
    : functions  function
        {
            struct CodeNode *function_declarations = $1;
            struct CodeNode *function_declaration = $2;
            struct CodeNode *node = new CodeNode;
            node->code = function_declarations->code + function_declaration->code;
            $$ = node;
        }
    ...
    | INT L_BRAKET NUMBER R_BRAKET IDENTIFIER 
        {
            ...
            struct CodeNode *node = new CodeNode;
            node->code = ".[] " + std::string($5) + ", " + std::string($3) + "\n";
            $$ = node;
        }
...
paramerter_dec:
    ...
    | INT IDENTIFIER COMMA paramerter_dec 
        {
            if (isDeclared($2)) {
            yyerror("Parameter redeclared");
            } else {
            symbolTable.push_back($2);
            struct CodeNode * node = new CodeNode;
            struct CodeNode * paramerter_dec = $4;
            node -> code = std::string(". ") + std::string($2) + std::string("\n");
            node -> code += paramerter_dec -> code;
            $$ = node;
            }
        } 
    | INT L_BRAKET R_BRAKET IDENTIFIER 
        {} 
    | INT L_BRAKET R_BRAKET IDENTIFIER COMMA paramerter_dec 
        {} 
    | %empty {struct CodeNode *node = new CodeNode;$$ = node;}


function
    : FUNC IDENTIFIER L_PAR  {symbolTable.clear();} paramerter_dec R_PAR L_CURLY statements R_CURLY 
        {
        std::string subS = "";
        int cnt = 0;
        int dotPlace = 0;
        ...
        struct CodeNode * paramerter_dec = $5;
        node -> code = std::string("func ") + std::string($2) + std::string("\n");
        node -> code += paramerter_dec -> code;
        for (int i = 0; i < paramerter_dec -> code.length(); i++) {
            if (paramerter_dec -> code.at(i) == '\n') {
            subS = paramerter_dec -> code.substr(dotPlace + 1, i - dotPlace - 1);
